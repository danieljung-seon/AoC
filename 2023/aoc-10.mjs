...
const i3 = `7-J-..FF.FLJJ7-7-L|7FFF7F7FF..|7-77.F77L---|7L-77-FF.LF-J77F-7L7L7|--.F-|.FFFF-|7-|.F--|FF.F.F77-L7-|7FF.FJ7FLL7-J7-|7|-L7-|7.7FF|-7-F.7777|
...
const getValidSymbols = (current, currentPos, neighbour, neighbourPos) => {
  switch (current) {
    case 'F':
      if (currentPos.x < neighbourPos.x) {
        return ['J', '7', '-'];
      }
      if (currentPos.x > neighbourPos.x) {
        return [];
      }
      if (currentPos.y < neighbourPos.y) {
        return ['|', 'J', 'L'];
      }
      if (currentPos.y > neighbourPos.y) {
        return [];
    ...
    case '.':
      return [];
    default:
      throw new Error(`Invalid symbol ${current}`);
  }
}

const isValidNeighbour = (current, currentPos, neighbour, neighbourPos) => {
  const validSymbols = getValidSymbols(current, currentPos, neighbour, neighbourPos);
  return validSymbols.includes(neighbour);
}
...
function parseMap(input, startReplaceMent) {
  const map = input
    .split('\n')
    .map(parseLine);
  ...
  if (!startReplaceMent) {
    throw new Error('Start replacement not provided');
  }

  map[y][x] = startReplaceMent;
...
function solve(map, x, y) {
  const visited = new Set();
  visited.add(`${x},${y}`);
  const stack = [];

  function walkMap(map, x, y, deep = 0) {
    ...
    for (const [nx, ny] of neighbours) {
      if (map[ny] && map[ny][nx] && isValidNeighbour(current, { x, y }, map[ny] && map[ny][nx], { x: nx, y: ny }) && !visited.has(`${nx},${ny}`)) {
        // console.log(`[${nx},${ny}]`, map[ny][nx], deep, stack.length);
        visited.add(`${nx},${ny}`);
        stack.push({ x: nx, y: ny, deep: deep + 1 });
      }
    }
    ...
    return walkMap(map, next.x, next.y, next.deep)
  }

  return walkMap(map, x, y);
}

// const data = parseMap(i1, 'F');
// const data = parseMap(i1b, 'F');
// const data = parseMap(i2, 'F');
const data = parseMap(i3, '|');
const result = solve(data.map, data.x, data.y);
